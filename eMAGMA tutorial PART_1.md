...
The output is a genes.out file and a genes.raw file that would be used in the next step.
   ...
   **Gene-set analysis**
#This step runs a competitive gene-set analysis as described in ref(xx) here we test for the association of genes within a set and phenotypes i.e MDD. The anlaysis counts for gene size and gene density. Here we use the networks files [normalised.tx] that were generated by refxxxx (see details of how the networks were build) and are freely provided. The network files are the gene sets definitions (by colors) for each tissue xxx, each file have 2 columns: gene set and gene ID. The second input on the gene-set anlaysis is the .raw outcome from the gene-based analysis [MDD_genes.raw]. 
